    ...
    <section className=' dark:bg-slate-700 rounded-md col-span-2 bg-slate-50 overflow-hidden'>
      ...
      <div className='grid grid-rows-3'>
        {data?.map((item) => (
          ...
          key={item.id}
          className="col-span-1 border-b last:border-b-0 block w-full h-full"
        >
          <div className="flex h-full w-full items-center justify-between px-4 py-3 hover:cursor-pointer hover:bg-teal-600 hover:text-white transition group">
            <div>
              ...
              <p className="text-xs">
                <span className="font-medium">{t("views")}</span>: {item.views}
              </p>
            </div>
