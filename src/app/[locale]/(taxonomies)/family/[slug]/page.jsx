...
export default async function Family({ params, searchParams }) {
  ...
  const { data } = await getData(`taxonomy/${params.slug}?type=Family`, params.locale)
  const child = await getPaginatedData(`taxonomy?type=Genus&parent_id=${data?.metadata?.id}`, params.locale, currentPage, TAXON_PER_PAGE)
